...
def showdata(bot, message, way_to_data):     #тут мы создаем список колод
    ...
    for i in range(len(pack_list)):
            btn = types.InlineKeyboardButton(text=pack_list[i], callback_data='showdata:' + str(pack_list[i]))
            markup.add(btn)
...
def open_pack(bot, call, way_to_data):
    ...
    if df.empty:    #если колода пустая
        ...
        for _, row in df.iterrows():
            words_list = words_list + '\n' + row['front_word'] + ' - ' + row['back_word'] + ' - ' + str(row['date_of_repeat']) + ' - ' + str(row['repeat_length']) #добавляем оба слова
